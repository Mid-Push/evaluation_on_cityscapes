...
class Resize(object):
    ...
    def __call__(self, pic, label=None):
        img = torchvision.transforms.Resize(self.size, interpolation=Image.BICUBIC)(pic)
        if label is None:
            return img,
        else:
            label = torchvision.transforms.Resize(self.size, interpolation=Image.NEAREST)(label)
            return img, label
