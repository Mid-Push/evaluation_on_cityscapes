...
def train_seg(args):
    ...
    normalize = transforms.Normalize(mean=info['mean'],
                                     std=info['std'])
    t = []
    t.append(transforms.Resize([143, 286])) # mine addin
    if args.random_rotate > 0:
        t.append(transforms.RandomRotate(args.random_rotate))
    if args.random_scale > 0:
        t.append(transforms.RandomScale(args.random_scale))
    t.extend([transforms.RandomCrop((256, 128)),
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if has_gt:
            ...
            pix_acc = (label == pred).mean()
            if iter % 100 == 0:
                logger.info('===> PixAcc {PixAcc:.3f}'.format(PixAcc=pix_acc))
    ...
    if has_gt: #val
        ...
        pixAcc = np.array(pix_acc_list).mean()
        classAcc = np.diag(hist) / hist.sum(axis=1)
        return round(np.nanmean(ious), 2), pixAcc, np.nanmean(classAcc)
...
def test_seg(args):
    ...
    out_dir = './results/' + '{}_{:03d}_{}'.format(args.arch, start_epoch, phase)
    ...
    if args.ms:
        ...
        mAP, PixAcc, ClsAcc = test(test_loader, model, args.classes, save_vis=False,
                   has_gt=phase != 'test' or args.with_gt, output_dir=out_dir)
    logger.info('mAP: %f', mAP)
    logger.info('PixAcc: %f', PixAcc)
    logger.info('ClsAcc: %f', ClsAcc)
    print('mAP: %f', mAP)
    print('PixAcc: %f', PixAcc)
    print('ClsAcc: %f', ClsAcc)
